...
def save_volume_as_vti(volume_data, filename):
    # Convertir le volume en un tableau numpy si ce n'est pas déjà fait
    if not isinstance(volume_data, np.ndarray):
        volume_data = np.array(volume_data)

    # Créer un objet VTK ImageData
    vtk_image = vtk.vtkImageData()
    # Check if the volume data has 4 dimensions (x, y, z, 3)
    if volume_data.ndim == 4 and volume_data.shape[3] == 3:
        # Reshape the volume data to 3D by combining the RGB channels
        volume_data = volume_data.reshape(volume_data.shape[0], volume_data.shape[1], volume_data.shape[2] * 3)
    
    vtk_image.SetDimensions(volume_data.shape[:3])
    vtk_image.SetSpacing(1.0, 1.0, 1.0)

    # Convertir le tableau numpy en un tableau VTK
    vtk_array = numpy_support.numpy_to_vtk(num_array=volume_data.ravel(), deep=True, array_type=vtk.VTK_FLOAT)
...
def test_img(pth, imtype, netG, nz=64, lf=4, periodic=False):
    ...
    # Enregistrer le volume au format VTI
    save_volume_as_vti(tif, pth + '.vti')
